...
- Every library photo is looked at once by `ImageVetter` (Claude Haiku, thumbnail size) as it is imported: `kind` (photo, illustration, screenshot, text_graphic, logo, other), a one-line `description`, and `vetted_at`. Screenshots, text graphics and logos are stored switched off with a `vetting_note` shown on the Photos page; uploads are described but never switched off. A `perceptual_hash` (dHash) within 6 bits of an existing photo marks a near-duplicate, stored switched off. Photos whose check failed stay unvetted, and unvetted photos are never used; `images:vet` (daily 04:30) retries them. `LIBRARY_IMAGES_ENABLED` gates library photos on posts.
- `ImageLibraryService::pickForPost()` runs first, at post creation, and also ranks photos by how many words their description shares with the post. It prefers an enabled photo from the same website page the post was written from, then Google and uploaded photos, then any; skips anything used in the last 21 days if something else is available; least recently used first. The rules live in the pure `ImagePicker`. The chosen URL goes straight into `media_urls` and `ai_metadata.image = {source, business_image_id}`.
